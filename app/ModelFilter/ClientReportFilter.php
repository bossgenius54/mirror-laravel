...
class ClientReportFilter extends Model
{
    private $items = null;
    ...
    private  function filterName(){
        ...
        $this->items->whereHas('relOrder', function($r) use ($request){
            $r->whereHas('relPositions', function($q) use ($request){
    ...
    private  function filterBranch(){
        ...
        $request = $this->request;
        $this->items->whereHas('relOrder', function($r) use ($request){
            $r->where('branch_id', $request->branch_id);
        });
    }
    ...
    private  function filterCreatedAt(){
        ...
        $request = $this->request;
        $this->items->whereHas('relOrder', function($r) use ($request){
            $r->whereBetween(DB::raw('DATE(updated_at)'), array($request->created_at_first, $request->created_at_second) );
        });
    }
}
