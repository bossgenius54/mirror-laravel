...
class StockManagerController extends Controller{
    ...
    function postCreate(Request $request){
        ...
        return redirect()->action("Lib\StockManagerController@getIndex")->with('success', 'Добавлен элемент списка "'.$this->title.'" № '.$item->id);
    }
    ...
    function postUpdate(Request $request, StockManager $item){
        ...
        return redirect()->action("Lib\StockManagerController@getIndex")->with('success', 'Изменен элемент списка "'.$this->title.'" № '.$item->id);
    }
