...
class AccounterController extends Controller{
    ...
    function postCreate(Request $request){
        ...
        return redirect()->action("Lib\AccounterController@getIndex")->with('success', 'Добавлен элемент списка "'.$this->title.'" № '.$item->id);
    }
    ...
    function postUpdate(Request $request, Accounter $item){
        ...
        return redirect()->action("Lib\AccounterController@getIndex")->with('success', 'Изменен элемент списка "'.$this->title.'" № '.$item->id);
    }
